...
def add_metadata_to_sections(sections: dict[str, str]) -> None:
    ...
    for name, content in sections.items():
        ...
        if metadata is None:
            ...
            tags = ["#status/active"]
            if "soul" in name or "profile" in name or "config" in name:
...
def mark_all_reviewed(sections: dict[str, str]) -> None:
    ...
    for name, content in sections.items():
        ...
        new_content = update_frontmatter(content, last_reviewed=datetime.now().date().isoformat())
...
def main():
    ...
    parser.add_argument(
        "--add-metadata", action="store_true", help="Add metadata to sections without it"
    )
    parser.add_argument(
        "--mark-reviewed", action="store_true", help="Mark all sections as reviewed today"
    )
