...
def _mock_supabase(configured_providers: list[str] = None):
    ...
    mock_sb = MagicMock()
    rows = [
        {"provider": p, "encrypted_key": "fake-ciphertext"} for p in (configured_providers or [])
    ]
...
class TestListKeys:
    ...
    def test_authenticated_no_keys_returns_empty(self):
        with (
            patch("api.services.key_store._supabase", return_value=_mock_supabase([])),
    ...
    def test_authenticated_with_keys_returns_providers(self):
        with (
            patch(
                "api.services.key_store._supabase",
                return_value=_mock_supabase(["openai", "anthropic"]),
            ),
            patch("api.services.key_store.decrypt_key", return_value="plaintext"),
        ):
            res = client.get("/user/keys", headers=_auth())
    ...
    def test_corrupted_key_appears_in_corrupted_providers(self):
        ...
        with (
            patch("api.services.key_store._supabase", return_value=_mock_supabase(["openai"])),
    ...
    def test_all_valid_providers_accepted(self):
        ...
        for provider in valid_providers:
            with (
                ...
                res = client.post(
                    f"/user/keys/{provider}", json={"key": "test-key"}, headers=_auth()
                )
                assert (
                    res.status_code == 200
                ), f"Expected 200 for provider {provider}, got {res.status_code}"
