...
def _make_encrypted_section(token_id: str, wallet_sig: bytes, section_name: str, content: bytes) -> bytes:
...
side_effect=lambda file_id, key, token_id, section_name="", expected_hash=None:
section_contents.get(section_name, b"# content"),
)
...
assert data["context_sections"]["soul"] == _SOUL_CONTENT.decode()
...
mocker.patch("api.routes.session.get_validator_contract_id", return_value="0.0.99000")
mocker.patch("api.routes.session._get_registered_file_id", return_value="0.0.99001")
mocker.patch("api.routes.session.token_id_to_evm_address", return_value="0" * 40)
mocker.patch("api.routes.session.pull_index", return_value=section_ids_with_dirs)
mocker.patch(
"api.routes.session.pull_section",
side_effect=lambda file_id, key, token_id, section_name="", expected_hash=None:
section_contents.get(section_name, b"# fallback"),
)
mocker.patch("api.routes.session.log_event", return_value="")
...
resp = client.post("/session/open", json={
"token_id": TOKEN_ID,
"wallet_signature_hex": "ab" * 32, # 32 bytes — wrong length
})
assert resp.status_code == 400

def test_invalid_tag_on_index_returns_401(self, mocker):
mocker.patch("api.routes.session.get_validator_contract_id", return_value="0.0.99000")
mocker.patch("api.routes.session._get_registered_file_id", return_value="0.0.99001")
mocker.patch("api.routes.session.token_id_to_evm_address", return_value="0" * 40)
mocker.patch("api.routes.session.pull_index", side_effect=InvalidTag())
...
assert "invalid wallet signature" in resp.json()["detail"].lower()

def test_invalid_tag_on_section_returns_401(self, mocker):
mocker.patch("api.routes.session.get_validator_contract_id", return_value="0.0.99000")
mocker.patch("api.routes.session._get_registered_file_id", return_value="0.0.99001")
mocker.patch("api.routes.session.token_id_to_evm_address", return_value="0" * 40)
mocker.patch("api.routes.session.pull_index", return_value={
"soul": "0.0.100001",
"user": "0.0.100002",
"symbiote": "0.0.100003",
"session_state": "0.0.100004",
})
mocker.patch("api.routes.session.pull_section", side_effect=InvalidTag())
...
def test_injection_detected_returns_403(self, mocker):
mocker.patch("api.routes.session.get_validator_contract_id", return_value="0.0.99000")
mocker.patch("api.routes.session._get_registered_file_id", return_value="0.0.99001")
mocker.patch("api.routes.session.token_id_to_evm_address", return_value="0" * 40)
mocker.patch("api.routes.session.pull_index", return_value={"soul": "0.0.100001"})
mocker.patch(
"api.routes.session.pull_section",
side_effect=ValueError("[integrity:soul] Content failed security scan"),
)
...
mocker.patch("api.routes.session.get_validator_contract_id", return_value=None)
...
def test_token_not_registered_returns_404(self, mocker):
mocker.patch("api.routes.session.get_validator_contract_id", return_value="0.0.99000")
mocker.patch("api.routes.session._get_registered_file_id", return_value="")
mocker.patch("api.routes.session.token_id_to_evm_address", return_value="0" * 40)
...
assert "no vault registered" in resp.json()["detail"].lower()
...
def _open_session(self, mocker) -> str:
...
mocker.patch("api.routes.session.pull_section", return_value=_SOUL_CONTENT)
mocker.patch("api.routes.session.log_event", return_value="")
...
def test_close_returns_200(self, mocker):
session_id = self._open_session(mocker)
resp = client.post("/session/close", json={"session_id": session_id})
assert resp.status_code == 200
data = resp.json()
assert data["session_id"] == session_id
assert "closed_at" in data

def test_close_evicts_session(self, mocker):
session_id = self._open_session(mocker)
...
def test_close_unknown_session_returns_404(self):
resp = client.post("/session/close", json={"session_id": "00000000-0000-0000-0000-000000000000"})
assert resp.status_code == 404
