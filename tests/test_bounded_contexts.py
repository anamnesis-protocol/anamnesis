...
def test_context_mapping_has_context():
    ...
    mapping = ContextMapping(
        package_name="test",
        contexts=[BoundedContext.AI_ENGINEERING]
    )
...
def test_infer_contexts_from_keywords_multiple():
    ...
    contexts = infer_contexts_from_keywords(
        "python_llm_api",
        ["python", "fastapi", "llm", "rag", "api"],
        "Python API for LLM with RAG"
    )
...
def test_filter_packages_by_context():
    ...
    filtered = filter_packages_by_context(
        packages,
...
def test_filter_packages_by_context_with_mappings():
    ...
    mappings = {
        "package1": ContextMapping(
            "package1",
            [BoundedContext.AI_ENGINEERING]
        ),
        "package2": ContextMapping(
            "package2",
...
def test_suggest_context_confidence():
    ...
    mapping1 = suggest_context_for_package(
        "specific_package",
        ["hedera", "hts"],
        "Hedera specific"
    )
