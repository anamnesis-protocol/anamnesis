...
class VaultMonitor:
    ...
    def _load_history(self) -> None:
        ...
        try:
            with open(self.metrics_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                self.metrics_history = [HealthMetric.from_dict(m) for m in data.get("metrics", [])]
                self.alerts = [HealthAlert.from_dict(a) for a in data.get("alerts", [])]
    ...
    def get_health_trend(self, days: int = 30) -> dict:
        ...
        recent = [m for m in self.metrics_history if datetime.fromisoformat(m.timestamp) > cutoff]
