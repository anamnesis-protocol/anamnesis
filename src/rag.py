...
def compute_tfidf_vector(tokens: list[str], idf: dict[str, float]) -> dict[str, float]:
    ...
    return {term: tf_score * idf.get(term, 0.0) for term, tf_score in tf.items()}
...
def rag_query_packages(
    ...
    if contexts:
        packages = [pkg for pkg in packages if pkg.bounded_context in contexts]
        if not packages:
            return []
...
def explain_match(query: str, pkg: PackageMetadata, similarity: float) -> dict:
    """
