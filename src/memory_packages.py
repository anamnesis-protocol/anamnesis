...
def extract_keywords(name: str, content: str, max_keywords: int = 12) -> list[str]:
 ...
 header = content[:500]
 # Extract words from headings (# lines) and first sentences
...
def extract_description(name: str, content: str) -> str:
 ...
 # Look for first non-empty, non-heading line after any H1
 for line in lines:
 stripped = line.strip()
...
def push_package(
 ...
 Returns:
 PackageMetadata with file_id populated
 """
...
def pull_package(
 ...
 Runs integrity verification (injection scan) and emits
 PACKAGE_ACCESSED event to HCS audit trail.
...
def pull_packages_parallel(
 ...
 Uses ThreadPoolExecutor — each worker creates its own Hedera client.
 Fetching N packages takes the time of the slowest single package
 rather than sum of all round-trips.
 ...
 pkg_name, content = future.result() # propagates exceptions
...
def pull_package_index(index_file_id: str, key: bytes, token_id: str) -> list[PackageMetadata]:
 ...
 AAD binding ensures the package index cannot be substituted with a section blob.
...
def push_all_packages(
 ...
 # Differential push: load stored hashes, preserve hashes for categories not in this run
 ...
 # Differential: skip upload if content hash unchanged
 ...
 # Push package index — skip if nothing changed and index already exists
 ...
 # Update vault index on HFS — vault index uses index_key (not package_key)
 _update_vault_index_with_packages(index_key, token_id, local, index_file_id)

 # Save local cache with content hashes for next differential push
