...
# These are the core identity files — loaded into model context at session start,
# injection-scanned on every pull, and registered in the ContextValidator contract.
...
# These are backup/sovereignty sections — bundled per directory, compressed + encrypted.
# NOT loaded into model context at session start. NOT injection-scanned.
# Purpose: sovereign backup of working files; recovery via vault_pull.py --dirs.
VAULT_DIRS: dict[str, str] = {
 "dir:research": "Research",
 "dir:projects": "02 - Projects",
 "dir:system": "System",
 # Full vault coverage — PDFs excluded by _DIR_BUNDLE_EXTENSIONS = (".md",)
 "dir:knowledge": "Knowledge", # knowledge summaries (AI-Eng, Arch, Python, etc.)
 "dir:stack": "Stack", # integration stack documentation
 "dir:personas": "Personas", # all persona files (Drake, Kai, Zara, Ravi, etc.)
 "dir:archive": "Archive", # session history
 "dir:root-files": ".", # root-level .md files only (CLAUDE.md, ONBOARDING.md, etc.)
}

# Per-directory push options — override defaults for specific dir bundles.
# "recursive": False → use glob("*") instead of rglob("*") to capture only direct children.
# Used for "dir:root-files" to avoid duplicating content from all other dir bundles.
...
# Different data stores use different derived keys from the same master secret.
# Compromise of one key does not compromise the others.
# ---------------------------------------------------------------------------
_INFO_SECTION = b"sovereign-ai-section-v1" # vault section content (SOUL, USER, etc.)
_INFO_INDEX = b"sovereign-ai-index-v1" # vault index JSON
_INFO_PACKAGE = b"sovereign-ai-package-v1" # memory packages (sessions, research, projects)
_INFO_LEGACY = b"sovereign-ai-context-v1" # original key — kept for backward compat

# ---------------------------------------------------------------------------
# Injection pattern detection (AI-Security.md — OWASP LLM01, LLM02)
# Patterns that should never appear in a trusted vault section.
# ---------------------------------------------------------------------------
_INJECTION_PATTERNS = [
...
def get_section_key(token_id: str) -> bytes:
 ...
 Distinct from index and package keys — compromise of section key does not
 expose index file IDs or package contents.
 """
 wallet_sig = get_wallet_signature(token_id)
...
def get_index_key(token_id: str) -> bytes:
 ...
 Compromise of the index key reveals only file IDs, not section content.
 """
 wallet_sig = get_wallet_signature(token_id)
...
# Each data type gets a distinct AAD string that binds the ciphertext to its
# logical purpose. Decrypting a section blob with index AAD raises InvalidTag.
# ---------------------------------------------------------------------------
...
def bundle_directory(
 ...
 If False, only include direct children via glob — used for
 "dir:root-files" to avoid duplicating subdirectory content.
...
def bundle_from_dict(files: dict[str, str]) -> bytes:
 ...
 Produces the same JSON format as bundle_directory() — base64-encoded values —
 but without any local file I/O. Used by the SaaS API so users can push
 arbitrary named bundles without needing a local directory structure.
...
def unbundle_to_dict(bundle_bytes: bytes) -> dict[str, str]:
 ...
 Inverse of bundle_from_dict(). Used by the SaaS API to return bundle content
...
def push_dir_section(
 ...
 Uses the section key (same as identity sections) with an AAD of
 b"dir:{dir_name}:{token_id}" to bind the ciphertext to this directory.
...
def push_section(
 ...
 Compression happens before encryption — ciphertext is smaller on HFS.
 """
 aad = _make_section_aad(section_name, token_id)
 payload = compress(content) # gzip before encrypt
...
def pull_section(
 ...
 raw = load_context(key, file_id, token_id, aad)
 content = decompress(raw) # backward-compat: pass-through if not compressed
...
def push_index(
 ...
 On session open, each section's plaintext is hashed and compared against this map
 before the content is accepted — detecting any tampering between push and pull.

 This implements the Content Hash Index sub-claim (patent non-provisional).
 Backward compatible: old vaults without '_section_hashes' skip hash verification.
 """
 aad = _make_index_aad(token_id)
 payload = dict(index) # shallow copy — don't mutate caller's dict
 if section_hashes:
...
def update_index(
 ...
 Used at session close to write back updated section hashes without creating
 a new HFS file — the index_file_id stays the same, so no contract re-registration
 is needed.
...
def pull_index(index_file_id: str, key: bytes, token_id: str) -> dict[str, str]:
 ...
 AAD binding ensures this ciphertext cannot be substituted with a section blob.
 """
 aad = _make_index_aad(token_id)
...
def load_local_index() -> dict:
 ...
 is present and wrong, raises ValueError — re-run vault_push.py to rebuild.
 """
...
def save_local_index(data: dict) -> None:
 ...
 The _integrity field is a SHA-256 over the canonical JSON of the data
 (sorted keys, no extra whitespace). Verified on load to detect tampering.
 """
...
def push_all(force_new: bool = False) -> dict[str, str]:
 ...
 Phase 1 — Identity sections (section_key):
 soul, user, symbiote, session_state
 Injection-scanned on pull. Loaded into model context at session start.

 Phase 2 — Directory bundles (section_key, dir: prefix):
 dir:research, dir:projects, dir:system
 Backup/sovereignty only. Not loaded into model context. Not injection-scanned.
 Differential push: bundle hash compared against last push; skip if unchanged.
 ...
 # Differential push: skip upload if content unchanged
 ...
 # Differential push at bundle level
 ...
 # Count files in bundle for display
 manifest = json.loads(bundle.decode("utf-8"))
 print(f" [{dir_name}] {len(manifest)} files ({len(bundle):,} bytes uncompressed)")
 dirs_changed = True
 file_id = push_dir_section(dir_name, dir_path, section_key, token_id,
 existing_file_id, recursive=recursive)
 ...
 # Update existing index — include section hashes for tamper detection (Content Hash Index)
 index_payload = {**all_section_ids, "_section_hashes": current_hashes}
 index_bytes = json.dumps(index_payload, indent=2).encode("utf-8")
 update_context(index_key, existing_index_file_id, index_bytes, token_id, _make_index_aad(token_id))
 index_file_id = existing_index_file_id
 print(f" Vault index updated -> {index_file_id}")
 else:
 # New index — embed section hashes for tamper detection (Content Hash Index)
 index_file_id = push_index(all_section_ids, index_key, token_id, section_hashes=current_hashes)
...
def _pull_section_task(args: tuple) -> tuple[str, bytes]:
 ...
 pull_section() would use the wrong AAD prefix for dir sections.
 """
 ...
 content = decompress(raw) # bundle is gzip-compressed JSON manifest
 else:
...
def pull_all(output_dir: Path | None = None) -> dict[str, bytes]:
 ...
 # Build fetch tasks — exclude metadata keys (packages pointer, _section_hashes map)
 ...
 section_name, content = future.result() # propagates exceptions
